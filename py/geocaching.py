...
class GeocacheApi:
    ...
    def __init__(self, username: str = None, password: str = None):
        ...
        self.__password = password or self.__PASSWORD
    ...
    def __signin(self):
        ...
        if match:
            ...
            print(
                f"Login succes: {self.__userdata['username']} (ID: {self.__userdata['accountId']}, Membership: {self.__userdata['membershipLevel']})")
            return True
        print("Login failed")
        return False
    ...
    def __get_results(self, longitude: float, latitude: float, count: int):
        ...
        response = self.__session.get(
            f"https://www.geocaching.com/api/proxy/web/search/v2?sort=distance&take={count}&origin={longitude},{latitude}")
...
def __main__():
    ...
    parser.add_argument('-c', '--count', default=1000, type=int,
                        help="Number of geocaches to find (max 1000)")

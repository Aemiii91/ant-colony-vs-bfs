...
class DFSBranchLimitAlgorithm : public BaseAlgorithm {
  public:
    /**
	 * Calculates the longest route given the branchLimit parameter and the graph and time interval assigned 
	 * to the class instance. 
     * Given that the provided graph is fully dense, will always return a route that ends at the starting node.
     * 
     * @param branchLimit Used to limit the amount of branches per node.
	 * @returns A vector containing the best computed route through the graph with the given initializations and parameters.
	 */
	 ...
	 * 
	 * @param timeInterval Used to initialize the time interval given in seconds.
  ...
  private:
	 ...
	 * @param currentPath Used to check if the generated branching nodes already exist in the current path of the iteration.
	 * @param branchLimit Used to limit the amount of branches per node.
	 * 
	 * @returns A vector containing the branching nodes for the given currentNode parameter.
	 */
	 ...
	 * 
	 * @returns True or False depending on whether the node exists in the currentPath parameter or not.
	 */
